  ...
  class Ripple < Base
    def process_blockchain(blocks_limit: 300, force: false)
      ...
      (current_ledger..latest_ledger).each do |ledger_index|
        ...
        deposits    = build_deposits(transactions, ledger_index)
        withdrawals = build_withdrawals(transactions, ledger_index)

        deposits.map { |d| d[:txid] }.join(',').tap do |txids|
          Rails.logger.info { "Deposit transactions in ledger #{ledger_index}: #{txids}" }
        end

        withdrawals.map { |d| d[:txid] }.join(',').tap do |txids|
          Rails.logger.info { "Withdraw transactions in ledger #{ledger_index}: #{txids}" }
        end

        update_or_create_deposits!(deposits)
        update_withdrawals!(withdrawals)
        update_height(ledger_index, latest_ledger)
